...
Describe the screen/route sequence and transitions. Use ASCII diagrams or a route list when no wireframe is available.
For services without a UI (HTTP API, CLI, worker), describe the route/subcommand surface as the equivalent navigation map.
...
- **Actions:** buttons/endpoints and their consequences
- **Validations:** screen/route-level validation rules
...
Describe the database tables or the Go `struct` types relevant to this module
(include the `json:`/`db:` tags when present — they reveal the serialization/persistence mapping).
...
|---|---|---|---|---|---|
| INT-001 | | REST API / gRPC / Queue | HTTP / gRPC / TCP / AMQP | Inbound / Outbound | |
...
- Minimum Go version: [X] (via `go.mod`, `go X.YY` directive)
- Database: [name and version] (`database/sql` driver / ORM)
- Target platform: [GOOS/GOARCH — e.g., linux/amd64, windows/amd64, darwin/arm64]
- Required dependencies (go.mod: packages and versions): [module@version]
...
- [ ] Tests (`go test ./...`) cover at least [X]% of the business code
