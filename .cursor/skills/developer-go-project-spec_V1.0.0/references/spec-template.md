...
Descreva a sequencia de telas/rotas e transicoes. Use diagramas ASCII ou lista de rotas se nao houver wireframe.
Em servicos sem UI (API HTTP, CLI, worker), descreva a superficie de rotas/subcomandos como mapa de navegacao equivalente.
...
- **Acoes:** botoes/endpoints e suas consequencias
- **Validacoes:** regras de validacao da tela/rota
...
Descreva as tabelas de banco de dados ou os `struct` Go relevantes para este modulo
(inclua as tags `json:`/`db:` quando presentes, pois revelam o mapeamento serializacao/persistencia).
...
|---|---|---|---|---|---|
| INT-001 | | API REST / gRPC / Fila | HTTP / gRPC / TCP / AMQP | Entrada / Saida | |
...
- Go versao minima: [X] (via `go.mod`, diretiva `go X.YY`)
- Banco de dados: [nome e versao] (driver `database/sql` / ORM)
- Plataforma alvo: [GOOS/GOARCH — ex.: linux/amd64, windows/amd64, darwin/arm64]
- Dependencias obrigatorias (go.mod: pacotes e versoes): [modulo@versao]
...
- [ ] Testes (`go test ./...`) cobrindo ao menos [X]% do codigo de negocio
