...
description: >
  Testes unitários com DUnitX em Delphi — fixtures, assertions, TestCase parametrizado, mocking com Delphi-Mocks e isolamento de dependências via injeção.
  Opera em DOIS MODOS: Modo Explícito (/tdd — cobertura completa do projeto) e Modo Automático
  (invocado por developer-delphi-coding-workflow após cada nova implementação).
  Padrão de nomenclatura: Test_<Metodo>_<Cenario>. Referências em references/dunitx-patterns.md.
model: sonnet
thinking: extended
