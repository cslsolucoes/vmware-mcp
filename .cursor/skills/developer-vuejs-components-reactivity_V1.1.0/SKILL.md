...
- Configuração de testes unitários ou de integração → usar `developer-web-testing-debugging`.
- Deploy, Docker ou CI/CD → usar `developer-web-packaging-deployment`.
- Orquestração de múltiplas skills → usar `developer-vuejs-master-orchestrator`.

## Dependências (skills prévias)

| Skill | Quando executar antes |
|-------|----------------------|
| `developer-vuejs-language-core` | Sempre — define padrão JS vs TS e nomenclatura base |
...
## §E11 — Recursos avançados de componentes (adicionado V1.1.0)

### defineEmits com validação

```vue
<script setup lang="ts">
// Emits tipados com validação
...
// Componente pai — provide
import { provide, ref } from 'vue'
const theme = ref<'light' | 'dark'>('light')
provide(themeKey, theme)

// Componente filho — inject com default
import { inject } from 'vue'
const theme = inject(themeKey, ref('light'))
const user = inject(userKey, { name: 'Guest', role: 'viewer' })
```

### Slots nomeados e scoped slots

```vue
<!-- Componente pai: DataTable.vue -->
<template>
  <table>
    <tbody>
      <tr v-for="row in rows" :key="row.id">
        <!-- Scoped slot — expõe row para o consumer -->
        <slot name="row" :row="row" :index="index" />
      </tr>
    </tbody>
    <!-- Slot padrão para conteúdo extra -->
...
### Teleport — modais fora do DOM pai

```vue
<template>
  <button @click="open = true">Abrir Modal</button>

  <!-- Renderiza dentro de <body>, fora do componente -->
  <Teleport to="body">
...
```vue
<!-- AsyncView.vue — carregamento com fallback -->
<template>
  <Suspense>
    <template #default>
...
const AsyncUserProfile = defineAsyncComponent({
  ...
  loadingComponent: () => import('./SkeletonLoader.vue'),
...
### v-model em componentes customizados (Vue 3.4+ defineModel)

```vue
<!-- InputField.vue — Vue 3.4+ -->
<script setup lang="ts">
const model = defineModel<string>({ required: true })
// equivale a: props.modelValue + emit('update:modelValue', ...)
</script>

<template>
  <input v-model="model" class="input-field" />
</template>

<!-- Consumer -->
<InputField v-model="username" />

<!-- Vue 3.3 e anterior (padrão manual) -->
<script setup lang="ts">
...
watch(query, async (newVal, oldVal) => {
  page.value = 1  // reset ao mudar query
  await buscarResultados(newVal)
}, { immediate: true, debounce: 300 })

// watch múltiplas fontes
watch([query, page], ([q, p]) => {
  console.log(`Busca: "${q}" — página ${p}`)
})

// watchEffect — rastreia dependências automaticamente
watchEffect(() => {
  document.title = query.value ? `Busca: ${query.value}` : 'Home'
})
```
...
- 1.1.0 (2026-04-24): E11 — P2: adicionadas seções §E11 (defineEmits tipado, provide/inject, scoped slots, Teleport, Suspense+AsyncComponent, defineModel Vue 3.4+, watch avançado); fix 3 refs quebradas.
